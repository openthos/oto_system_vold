...
void DirectVolume::getMountpoint(int part, char buffer[])
{
    if (part)
        snprintf(buffer, PATH_MAX, "%s_%d", mFuseMountpoint, part);
    else
        strcpy(buffer, mMountpoint);

    SLOGD("DirectVolume::getMountpoint() return a partition mountpoint:(%s)", buffer);
}
